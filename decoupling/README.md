...
**[MEME: Boromir "One does not simply"]**
```
ONE DOES NOT SIMPLY
HANDLE EVERY FEATURE COMBINATION WITH @IF BLOCKS
```
...
export class SelectableRowDirective {
  ...
  select() { this.isSelected.set(true); }
  deselect() { this.isSelected.set(false); }
  toggle() { this.isSelected.update(v => !v); }
}
...
```
Boyfriend (looking back): Your codebase
Girlfriend (ignored): Clean architecture
Other woman: <tr selectableRow hoverHighlight keyboardNav copyOnClick ariaDescribed>
```
...
<tr selectableRow hoverHighlight keyboardNav copyOnClick></tr>
...
**[MEME: "Friendship ended"]**
```
FRIENDSHIP ENDED WITH
<tr selectableRow hoverHighlight keyboardNav copyOnClick>

NOW
<tr interactiveLogRow>
IS MY BEST FRIEND
```
...
export class InteractiveLogRowDirective {
  private selectable = inject(SelectableRowDirective, { self: true });
  private keyboard = inject(KeyboardNavDirective, { self: true });
...
**[MEME: Pam "They're the same picture" - subverted]**
```
Corporate needs you to find the difference between:

[Image 1]: Coupling scattered everywhere
[Image 2]: Coupling isolated in one place

Pam: "They're... actually very different and one is much better"
```
...
**[MEME: "I made this / You made this" - extended]**
```
Panel 1 - AI: "I made this refactor"
Panel 2 - Me: "You made this refactor"
Panel 3 - Me: "...following my design doc"
Panel 4 - Me: "...after two reviews"
Panel 5 - Me: "...with an implementation plan"
Panel 6 - Me: "I made this refactor"
```
